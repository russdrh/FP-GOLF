                  ...
                  unsigned long maxlen)
{
    ...
    my_memcpy(ctx->key, seed, 32);
    
    my_memcpy(ctx->ctr, diversifier, 8);
...
seedexpander(AES_XOF_struct *ctx, unsigned char *x, unsigned long xlen)
{
    ...
    while ( xlen > 0 ) {
        if ( xlen <= (16U-ctx->buffer_pos) ) { // buffer has what we need
            my_memcpy(x+offset, ctx->buffer+ctx->buffer_pos, xlen);
            ctx->buffer_pos += xlen;
        ...
        my_memcpy(x+offset, ctx->buffer+ctx->buffer_pos, 16-ctx->buffer_pos);
                 ...
                 int security_strength)
{
    ...
    my_memcpy(seed_material, entropy_input, 48);
...
randombytes(unsigned char *x, unsigned long long xlen)
{
    ...
    while ( xlen > 0 ) {
        ...
        if ( xlen > 15 ) {
            my_memcpy(x+i, block, 16);
            i += 16;
            xlen -= 16;
        }
        else {
            my_memcpy(x+i, block, xlen);
            xlen = 0;
        }
    }
                       ...
                       unsigned char *V)
{
    ...
    my_memcpy(Key, temp, 32);
    my_memcpy(V, temp+32, 16);
}
