...
int
crypto_expand(const unsigned char *sk, fpr *expanded_key)
{
	// randombytes_init((unsigned char*)seed_c, NULL, 256);
	TEMPALLOC union {
		uint8_t b[72 * 512];
		uint64_t dummy_u64;
		fpr dummy_fpr;
	 ...
	 */
	Zf(expand_privkey)(expanded_key, f, g, F, G, 9, tmp.b);
	// printf("key[0] = %lf\n", expanded_key[0]);
	// printf("key[256] = %lf\n", expanded_key[256]);
	return 0;
}
	...
	const unsigned char *sk, const unsigned char *seed_c, const unsigned char *nonce_c)
{
	...
	// memset(G, 5, 512 * sizeof(int8_t));
	// memset(f, 5, 512 * sizeof(int8_t));
	// memset(g, 5, 512 * sizeof(int8_t));
	// memset(F, 5, 512 * sizeof(int8_t));
	if (!Zf(complete_private)(G, f, g, F, 9, tmp.b)) {
		return -1;
	...
	inner_shake256_inject(&sc, nonce_c, NONCELEN);
	...
	// randombytes(seed, sizeof seed);
	inner_shake256_init(&sc);
	inner_shake256_inject(&sc, seed_c, SEEDLEN);
	inner_shake256_flip(&sc);
	...
	sig_len ++;
	my_memmove(sm + 2 + NONCELEN, m, mlen);
	sm[0] = (unsigned char)(sig_len >> 8);
	sm[1] = (unsigned char)sig_len;
	my_memcpy(sm + 2, nonce_c, NONCELEN);
	my_memcpy(sm + 2 + NONCELEN + mlen, esig, sig_len);
	...
	const unsigned char *pk)
{
	 ...
	 * Return plaintext.
	 */
	my_memmove(m, sm + 2 + NONCELEN, msg_len);
	*mlen = msg_len;
	return 0;
}